...
func (w *worker) newWorkLoop(recommit time.Duration) {
	var (
		...
		timestamp   int64      // timestamp for each round of mining.
			...
			log.Info("Mod: Submit a new mining work")
	timer := time.NewTimer(0)
	<-timer.C // discard the initial tick

	// commit aborts in-flight transaction execution with given signal and resubmits a new one.
	commit := func(noempty bool, s int32) {
			log.Info("Mod: commit aborts in-flight transaction execution with given signal and resubmits a new one.")
		if interrupt != nil {
...
func (w *worker) commitTransactions(txs *types.TransactionsByPriceAndNonce, coinbase common.Address, interrupt *int32) bool {
	...
	for {
		...
		case core.ErrNonceTooHigh:
			...
			log.Info("Mod:Skipping account with high nonce", "sender", from, "nonce", tx.Nonce())
			txs.Pop()
...
func (w *worker) commit(uncles []*types.Header, interval func(), update bool, start time.Time) error {
	...
	log.Info("Mod: Copying receipts to avoid interation between different tasks")
	receipts := make([]*types.Receipt, len(w.current.receipts))
	for i, l := range w.current.receipts {
		receipts[i] = new(types.Receipt)
		*receipts[i] = *l
	}
	log.Info("Mod: Copying current state")
